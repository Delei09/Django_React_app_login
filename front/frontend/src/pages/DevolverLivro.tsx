...
const DevolverLivros = () => {
    ...
    const handleClickOpen = (id : string) => {
      ...
      const handleClose = (opcao : boolean) => {
          if(opcao){
            const token = localStorage.getItem('token')
            const urlDevolver = `http://127.0.0.1:8000/devolver/`
            const livro = {"id_livro": idLivro}
            axios.post(urlDevolver, livro, {
                headers:{
                    'Authorization': `Bearer ${token}`
                }
            })
    ...
    return(
        <div className='container' >
            <Header pagina='Devolver Livros' itens={['Meus Dados','Dashboard', "Usuarios", "Livros" ,"Emprestar Livros" , "Devolver Livros", "Logoff"]} usuario={state.first_name}   ></Header>
            <p className='titulo' >Devolver Livros <LayersIcon className='icone'  /> </p>
                                         ...
                                         id={livro.id}  onClick={ (e : React.MouseEvent) => {
                                            ...
                                            >
                                                <DialogTitle id="alert-dialog-title">
                                                     {`Tem certeza que deseja devolver o livro ${idLivro}`}
                                                </DialogTitle>
                                                <DialogContent>
